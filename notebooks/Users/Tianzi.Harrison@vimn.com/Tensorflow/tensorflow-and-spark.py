...
# MAGIC 
# MAGIC *An example of integrating Apache Spark with a Deep Learning library*
# MAGIC 
# MAGIC This notebook takes an existing neural network (INCEPTION-3) and applies it to a corpus of images.  This notebook requires that TensorFlow be installed on the cluster; please see the provided Init Script for installing TensorFlow.
# MAGIC 
# MAGIC Spark integration works as follows:
# MAGIC * We define TensorFlow operations as methods, to be used within Spark tasks.
# MAGIC * We broadcast the model for use within Spark tasks.
# MAGIC * We parallelize a list of image URLs.
# MAGIC * Using Spark, we process the image URLs in parallel:
# MAGIC   * Load image.
# MAGIC   * Run inference on the image using TensorFlow to predict the image contents.
# MAGIC 
# MAGIC This is a port of tutorials published by the TensorFlow team:
# MAGIC * A description is available here: http://www.tensorflow.org/versions/master/tutorials/image_recognition/index.html
# MAGIC * The original code is available from the TensorFlow repo, under the Apache 2.0 license: https://github.com/tensorflow/tensorflow/blob/master/tensorflow/models/image/imagenet/classify_image.py
# MAGIC * Thanks to the TensorFlow team for providing the original examples!
